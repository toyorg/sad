    ...
    try {
        ...
        progress(request(data['js']['download_cmd']), {})
            .on('progress', function (state) {
                log('Downloaded: ' + (state.size.transferred / 1000000).toFixed(2) + ' MB/' + (state.size.total / 1000000).toFixed(2) + ' MB | ' + (state.percent * 100).toFixed(2) + ' % | ' + (state.speed * 0.000001).toFixed(2) + ' Mb/s | ETA ' + state.time.remaining + ' s | Elapsed: ' + state.time.elapsed + ' s');
            })
    ...
    get_token(mac).then(data => {
        ...
        get_profile(token, moment().unix(), mac).then(() => {
            inquirer
                ...
                .then(channel => {
                    get_channel_epg(channel.id, moment().subtract(1, 'd').format('YYYY-MM-DD'), 1, token, mac).then(data => {
                        var int;
                        for (int = 2; int <= Math.ceil(data); int++) {
                            get_channel_epg(channel.id, moment().subtract(1, 'd').format('YYYY-MM-DD'), int, token, mac);
                        }
                    }).then(() => {
                        get_channel_epg(channel.id, moment().format('YYYY-MM-DD'), 1, token, mac).then(data => {
                            var int;
                            for (int = 2; int <= Math.ceil(data); int++) {
                                get_channel_epg(channel.id, moment().format('YYYY-MM-DD'), int, token, mac);
                            }
                        }).then(() => {
                            get_profile(token, moment().unix()) // second time for get all TV shows to array ?? need to fix then()
                                .then(prog => {
